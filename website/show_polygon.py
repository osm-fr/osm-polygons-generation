...
if rel_id != -1 and name != "" and osm_results and user_results:
    ...
    for o_res in osm_results:
        ...
        for u_res in user_results:
            show("    <td>")
            show("<a href='get_wkt.py?id=%d&amp;params=%s&amp;name=%s'>WKT</a>\n" % (rel_id, o_res["params"], u_res["name"]))
            show("<a href='get_geojson.py?id=%d&amp;params=%s&amp;name=%s'>GeoJSON</a>\n" % (rel_id, o_res["params"], u_res["name"]))
            show("<a href='get_poly.py?id=%d&amp;params=%s&amp;name=%s'>poly</a>\n" % (rel_id, o_res["params"], u_res["name"]))
            show("<a href='get_image.py?id=%d&amp;params=%s&amp;name=%s'>image</a>\n" % (rel_id, o_res["params"], u_res["name"]))
            show("    </td>")
        show("  </tr>\n")
    show("</table>\n")
