...
class OsmBin:
    ...
    def WayGetGeom(self, WayId):
        dta = self.WayGet(WayId)
        geom = []
        for nd in dta["nd"]:
            node = self.NodeGet(nd)
            if node:
    ...
    def RelationGetWayGeom(self, RelationId, exclude_roles = [], RecurControl = []):
        rel = self.RelationGet(RelationId)
        if rel is None:
            return rel
        dta = {}
        for m in rel["member"]:
            if m["type"] == "node" or m["role"] in exclude_roles:
                pass
            elif m["type"] == "way":
                dta[m["ref"]] = self.WayGetGeom(m["ref"])
            elif m["type"] == "relation":
                if m["ref"] in RecurControl:
                    continue
                dta.update(self.RelationGetWayGeom(m["ref"], exclude_roles, RecurControl+[RelationId]))
        return dta
...
if __name__=="__main__":
    ...
    if sys.argv[1]=="--read":
        ...
        if sys.argv[3]=="relation_geom":
            res = i.RelationGetWayGeom(int(sys.argv[4]), ["subarea", "land_area", "disputed_exclave"])
